...
- describe 测试套件：描述, decribe会形成一个作用域 (用于分组)
- it 测试用例：定义了一组关于测试期望的方法,它接收测试名称和一个含有测试期望的函数
- expect 判定条件：用来创建断言 当检查点失败时，继续往下执行
- toEqual 断言结果
...
- not 否定断言
- toBe 断言基元是否相等或对象是否共享相同的引用
- toEqual 断言实际值是否等于接收到的值或具有相同的结构，如果它是一个对象（递归地比较它们）
- toBeDefined 断言该值不等于undefined
- toBeUndefined 断言该值等于undefined
- toBeTruthy 断言该值在转换为布尔值时为true
- toBeFalsy 断言该值在转换为布尔值时为false
- toBeNull 断言如果某物是null
- toBeNaN  断言如果某物是NaN
- toBeTypeOf 断言实际值是否属于接收类型  ('bigint' | 'boolean' | 'function' | 'number' | 'object' | 'string' | 'symbol' | 'undefined')
- toBeInstanceOf 断言实际值是否是接收类的实例
- toBeGreaterThan 断言实际值是否大于接收到的值
- toBeGreaterThanOrEqual 断言实际值是否大于接收到的值或等于它
- toBeLessThan 断言实际值是否小于接收到的值
- toBeLessThanOrEqual 断言实际值是否小于接收到的值 1 或等于它
- toContain 断言实际值是否在数组中。toContain也可以检查一个字符串是否是另一个字符串的子字符串
- toContainEqual 断言具有特定结构和值的项是否包含在数组中
- toHaveLength 断言对象是否具有.length属性并将其设置为某个数值
- toHaveProperty 断言提供的引用处的属性是否key存在于对象
